...
**Hire AI contractors into your engineering team.**
**Identity. Scope. Audit. Kill switch.**

[![License: Apache 2.0](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)
[![Status: Alpha](https://img.shields.io/badge/Status-Alpha-orange.svg)](#status)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](CONTRIBUTING.md)

[Quickstart](#quickstart) • [Why](#why-procuracy) • [How it works](#how-it-works) • [Templates](#templates) • [Security](#security-model) • [Comparisons](#comparisons)

</div>

---

> **One command to hire an AI contractor that lives in the tools your team already uses.**
> No new dashboard. No SaaS dependency. No vendor lock-in. Your infra, your data, your kill switch.

```bash
$ procuracy hire aria --template docs-maintainer --repo acme/docs
✓ Provisioned aria@acme.com
✓ Created GitHub user aria-acme (scoped to acme/docs, PR-only)
✓ Added @aria to Slack #engineering and #aria-log
✓ Created Linear user on team eng
✓ Wrote contract to ./contractors/aria/procuracy.yaml
✓ Posted welcome to #engineering

aria is ready. Assign her a Linear ticket to get started.
```

That's the entire onboarding flow. **Five minutes from `git clone` to a working AI contractor on your repo.**

---

## Why procuracy

Today, every team that wants AI to do real work in their org rebuilds the same five things from scratch:
...
**`procuracy` is the missing layer between "agent runtime" and "real org adoption."** It doesn't replace Claude Code, OpenHands, or any other agent — it *wraps* them with the org integration that makes them deployable in environments that need accountability, audit, and reversibility.
...
**Read the file once and you know exactly who this contractor is, what it can touch, when it works, what it costs, and how to fire it.** Versioned in git, reviewed in PRs, auditable forever.

## How it works

`procuracy` has four primitives. That's the entire conceptual surface.

### 1. Identity
The contractor has its own real, scoped accounts on the tools your team already uses. **Not a shared bot.** Not a wrapper around a human's credentials. A separate, auditable, revocable principal — exactly like a human contractor.
...
**Capability-based security, not instruction-based security.** No clever prompt injection can grant a capability the adapter doesn't have.
...
- **Local JSONL** — append-only log on disk, tamper-evident, exportable for compliance
...
```bash
# Install (single static binary, no dependencies)
curl -sSf https://procuracy.dev/install.sh | sh

# Authenticate to your org's tools (one-time, OAuth in browser)
procuracy auth github
procuracy auth slack
procuracy auth linear
procuracy auth anthropic

# Pick a template and hire your first contractor
procuracy init --template docs-maintainer
procuracy hire ./contractors/aria

# Start the runtime loop
procuracy start ./contractors/aria

# Watch it work
procuracy logs ./contractors/aria --follow
```
...
Initial templates shipped with the project:
...
Want to contribute a template? See [`CONTRIBUTING.md`](CONTRIBUTING.md). Templates are just directories with a `procuracy.yaml` and prompts — no code required.
...
The most overlooked truth about AI in the workplace is that **the bottleneck isn't model capability, it's trust infrastructure.** Models got good enough 18 months ago. What's still missing is the framework to onboard them as accountable participants — with identity, scope, audit, and a kill switch.

`procuracy` is that framework. It's intentionally small, intentionally opinionated, intentionally OSS, and intentionally engineered around the constraint that **no team should have to choose between AI productivity and operational accountability.**

## Status

`procuracy` is **alpha**. The manifest spec is stable. The CLI surface is stable. Adapter coverage is partial — GitHub and Slack are the most complete; Linear is in progress; Jira, Notion, and email are stubs. Production use at your own risk; contributions welcome to expand adapter coverage.
...
Read [`CONTRIBUTING.md`](CONTRIBUTING.md) and [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md). Open an issue before starting non-trivial work so we can avoid duplication.
...
**Hire your first AI contractor in 5 minutes.**
**[Get started](docs/quickstart.md) • [Read the manifest spec](docs/manifest-spec.md) • [Browse templates](examples/)**
